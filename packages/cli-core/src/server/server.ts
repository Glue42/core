...
export class CoreDevServer {
    ...
    public async setup(config: CliConfig, logger: Logger): Promise<CoreDevServer> {
        ...
        this.logger.info(`Setting up server with config: ${JSON.stringify(config, null, 2)}`);
    ...
    private bridgeApp(request: TracedRequest, next: express.NextFunction, definition: CliServerApp, response: express.Response): void {
        if (request.headers.accept?.includes("html")) {
            this.logger.trace(`[${request.id}] Request to ${definition.route} classified as html, fetching html...`);
            this.getAppHtml(definition, response, request.id);
            return;
        }

        this.logger.trace(`[${request.id}] Request to ${definition.route} classified as non-html, direct proxying...`);
    ...
    private getAppHtml(definition: CliServerApp, response: express.Response, requestId: string): void {
    ...
    private setUpGlueAssets(): void {
        const glueAssets = [
            { route: "/glue/worker.js", resolveWith: this.config.glueAssets.sharedWorker },
            { route: "/glue/gateway.js", resolveWith: this.config.glueAssets.gateway },
            { route: "/glue/config.json", resolveWith: this.config.glueAssets.config }
        ];
    ...
    private findApp(requestId: string, url: string, referrerUrl: string, excludeRoot?: boolean): CliServerApp {
        ...
        return this.config.server.apps.find((app) => {
            ...
            if (referrer && app.route === referrer) {

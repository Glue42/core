...
export class Logger implements Glue42Core.Logger.API {
    ...
    public log(message: string, level?: Glue42Core.LogLevel) {
    ...
    private publishMessage(level: Glue42Core.LogLevel, message: string) {
        ...
        if (this.canPublish(level, this.publishLevel())) {
            if (Logger.Interop?.methods({ name: Logger.InteropMethodName })) {
